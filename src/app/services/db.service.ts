...
//bala
 ...
 scrolled(data){
  ...
  if(data.detail.scrollTop < 50){
    ...
    // var refElement = $('#counters2_test');
    // if(refElement.position()!=undefined){
    //     if (refElement.position().top <= scrollPos + 100 ) {
    //       const elmnt = document.getElementById("counters2_test");
    //       let text = elmnt.offsetHeight
    //       if(text + refElement.position().top > 0){
    //         // this.counter_value = true;
    //         $(function() {
    //           function count($this){
    ...
    //         // value++;
    //         // document.getElementById('count_title_1').value = value;
    //       }
    //     }
    // }
}
...
isInViewport(class_name){
  ...
  img_style(data, type) {
    if (type == 'color') {
      ...
      return { 'background' : 'url('+ this.product_img(data) +') no-repeat',"min-height": "160px","background-size": "cover","position":"relative"};
    } else if(type == 'width'){
      return {'width':data};
    }else if(type == 'margin'){
      return {'margin':data};
